...
static int print_sms_to_pos(struct lgsm_handle *lh, int index)
{
   
   struct lgsm_sms_delete sms_del;
   int pos_fd = get_posfd();
   printf("get pos_fd=%d\n",pos_fd);
   lgsm_sms_read(lh,index);
   sms_del.index = index;
   //sleep(3);
   //lgsm_sms_delete(lh, &sms_del);
   return 0;
}
		...
		struct gsmd_evt_auxdata *aux)
{
	...
	if (aux->u.sms.inlined) {
		...
		printf("EVENT: Incoming SMS stored at location %i\n",
                ...
                sms_del.index = aux->u.sms.index;
                sleep(3);
                lgsm_sms_delete(lh, &sms_del);
#endif
                print_sms_to_pos(lh,aux->u.sms.index);
        }
	return 0;
}

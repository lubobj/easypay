...
#define PHONENUMBER 14
#define DATE_LENGTH 9
#define TIME_LENGTH 9
#define ZONE_LENGTH 7
struct pos_sms_format{
	char phonenumber_buf[PHONENUMBER];
	char date_buf[DATE_LENGTH];
	char time_buf[TIME_LENGTH];
	char zone_buf[ZONE_LENGTH];
}__attribute__ ((packed));

static int print_sms_to_pos(struct pos_sms_format *p_sms,struct gsmd_sms payload)
{
    int i;
	char *payload_temp;
	unsigned char enter[]={'\n'};
	...
	unsigned char signname[]={0xc7,0xa9,0xc3,0xfb,0x3a,0x0a};
	char *sms_print = NULL;
	int pos_fd = get_posfd();
	int rc;
	char* gbk_string = NULL;
	printf("phonenumber=%s\n",p_sms->phonenumber_buf);
	printf("date=%s\n",p_sms->date_buf);
	printf("time=%s\n",p_sms->time_buf);
	printf("zone=%s\n",p_sms->zone_buf);
	printf("payload length=%d\n",payload.length);
	//sms_print = malloc(14+1+9+1+9+1+7+1+payload.length);
	//the length of sms_print include PHONENUMBER+DATE_LENGTH+TIME_LENGTH+ZONE_LENGTH+3+SMS
	//sms_print = malloc(sizeof(companyname)+PHONENUMBER+1+DATE_LENGTH+1+TIME_LENGTH+1+ZONE_LENGTH+1+payload.length+1+sizeof(signname));
	sms_print = malloc(sizeof(companyname)+PHONENUMBER+1+DATE_LENGTH+1+TIME_LENGTH+1+payload.length+1+sizeof(signname));
    memcpy(sms_print,companyname,sizeof(companyname));
    memcpy(sms_print+sizeof(companyname),p_sms->phonenumber_buf,PHONENUMBER);
    memcpy(sms_print+sizeof(companyname)+PHONENUMBER,enter,1);
	memcpy(sms_print+sizeof(companyname)+PHONENUMBER+1,p_sms->date_buf,DATE_LENGTH);
	memcpy(sms_print+sizeof(companyname)+PHONENUMBER+1+DATE_LENGTH,enter,1);
	memcpy(sms_print+sizeof(companyname)+PHONENUMBER+1+DATE_LENGTH+1,p_sms->time_buf,TIME_LENGTH);
	memcpy(sms_print+sizeof(companyname)+PHONENUMBER+1+DATE_LENGTH+1+TIME_LENGTH,enter,1);
	//memcpy(sms_print+sizeof(companyname)+PHONENUMBER+1+DATE_LENGTH+1+TIME_LENGTH+1,p_sms->zone_buf,ZONE_LENGTH);
	//memcpy(sms_print+sizeof(companyname)+PHONENUMBER+1+DATE_LENGTH+1+TIME_LENGTH+1+ZONE_LENGTH,enter,1);

    if (payload.coding_scheme == ALPHABET_DEFAULT) {
		payload_temp = malloc(payload.length);
		unpacking_7bit_character(&payload, payload_temp);
		printf("sms:\"%s\"\n", payload_temp);
#if 0
	    memcpy(sms_print+sizeof(companyname)+PHONENUMBER+1+DATE_LENGTH+1+TIME_LENGTH+1+ZONE_LENGTH+1,payload_temp,payload.length);
	    memcpy(sms_print+sizeof(companyname)+PHONENUMBER+1+DATE_LENGTH+1+TIME_LENGTH+1+ZONE_LENGTH+1+sizeof(payload.length),enter,1);
	    memcpy(sms_print+sizeof(companyname)+PHONENUMBER+1+DATE_LENGTH+1+TIME_LENGTH+1+ZONE_LENGTH+1+payload.length+1,signname,sizeof(signname));
#endif
		//
	    memcpy(sms_print+sizeof(companyname)+PHONENUMBER+1+DATE_LENGTH+1+TIME_LENGTH+1,payload_temp,payload.length);
	    memcpy(sms_print+sizeof(companyname)+PHONENUMBER+1+DATE_LENGTH+1+TIME_LENGTH+1+sizeof(payload.length),enter,1);
	    memcpy(sms_print+sizeof(companyname)+PHONENUMBER+1+DATE_LENGTH+1+TIME_LENGTH+1+payload.length+1,signname,sizeof(signname));
		free(payload_temp);
    } else if (payload.coding_scheme == ALPHABET_8BIT)
		printf("8-bit encoded data\n");
	else if (payload.coding_scheme == ALPHABET_UCS2){
		printf("Unicode-16 encoded text\n");
		gbk_string = malloc(payload.length);
		rc = u2g(payload.data,payload.length,gbk_string,payload.length);
#if 0
	    memcpy(sms_print+sizeof(companyname)+PHONENUMBER+1+TIME_LENGTH+1+DATE_LENGTH+1+ZONE_LENGTH+1,gbk_string,payload.length);
	    memcpy(sms_print+sizeof(companyname)+PHONENUMBER+1+DATE_LENGTH+1+TIME_LENGTH+1+ZONE_LENGTH+1+payload.length,enter,1);
	    memcpy(sms_print+sizeof(companyname)+PHONENUMBER+1+DATE_LENGTH+1+TIME_LENGTH+1+ZONE_LENGTH+1+payload.length+1,signname,sizeof(signname));
#endif
	    memcpy(sms_print+sizeof(companyname)+PHONENUMBER+1+TIME_LENGTH+1+DATE_LENGTH+1,gbk_string,payload.length);
	    memcpy(sms_print+sizeof(companyname)+PHONENUMBER+1+DATE_LENGTH+1+TIME_LENGTH+1+payload.length,enter,1);
	    memcpy(sms_print+sizeof(companyname)+PHONENUMBER+1+DATE_LENGTH+1+TIME_LENGTH+1+payload.length+1,signname,sizeof(signname));
	   // memcpy(sms_print+14+1+9+1+9+1+7+1,payload.data,payload.length);
		free(gbk_string);
	} 
	//write(pos_fd,sms_print,sizeof(companyname)+PHONENUMBER+1+DATE_LENGTH+1+TIME_LENGTH+1+ZONE_LENGTH+1+payload.length+1+sizeof(signname));
	write(pos_fd,sms_print,sizeof(companyname)+PHONENUMBER+1+DATE_LENGTH+1+TIME_LENGTH+1+payload.length+1+sizeof(signname));
	free(sms_print);
    for(i=0;i<10;i++)
		write(pos_fd,enter,1);
	write(pos_fd,cutCommand,sizeof(cutCommand));
}
...
static int sms_msghandler(struct lgsm_handle *lh, struct gsmd_msg_hdr *gmh)
{
	...
	struct pos_sms_format *sms_buf = malloc(sizeof(struct pos_sms_format));
	int i = 0;
	...
	case GSMD_SMS_READ:
		...
		sprintf(sms_buf->phonenumber_buf,"%s%s",((sms->addr.type & __GSMD_TOA_TON_MASK) ==GSMD_TOA_TON_INTERNATIONAL) ? "+" : "",sms->addr.number);
		sprintf(sms_buf->date_buf,"%i%i-%i%i-%i%i",sms->time_stamp[0] & 0xf,sms->time_stamp[0] >> 4,sms->time_stamp[1] & 0xf,sms->time_stamp[1] >> 4,sms->time_stamp[2] & 0xf,sms->time_stamp[2] >> 4);
		sprintf(sms_buf->time_buf,"%i%i:%i%i:%i%i",sms->time_stamp[3] & 0xf,sms->time_stamp[3] >> 4,sms->time_stamp[4] & 0xf,sms->time_stamp[4] >> 4,sms->time_stamp[5] & 0xf,sms->time_stamp[5] >> 4);
		sprintf(sms_buf->zone_buf,"GMT%c%i",(sms->time_stamp[6] & 8) ? '-' : '+',(((sms->time_stamp[6] << 4) |(sms->time_stamp[6] >> 4)) & 0x3f) >> 2);
		if (sms->payload.coding_scheme == ALPHABET_DEFAULT) {
			printf("alphabet_default\n");
		...
		print_sms_to_pos(sms_buf,sms->payload);
...
static int call_msghandler(struct lgsm_handle *lh, struct gsmd_msg_hdr *gmh)
{
	...
	case GSMD_VOICECALL_CTRL:
		ret = (int*)((char *)gmh + sizeof(*gmh));
		 ...
		 if((*ret))
		 	syslog(LOG_USER|LOG_INFO, "+CME ERROR %d\n",*ret);
		 else
		 	syslog(LOG_USER|LOG_INFO, "OK\n");
		pending_responses --;
		break;
